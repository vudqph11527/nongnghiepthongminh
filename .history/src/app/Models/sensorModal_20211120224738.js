...
module.exports = {
  ...
  createDataSensor: (sensorData, callback) => {
    ...
    AS tmp WHERE NOT EXISTS (SELECT temperature, humidity,soidHumidity,soidPH,lightIntensity FROM devices WHERE 
      temperature = ${sensorData.temperature} AND humidity = ${sensorData.humidity} AND soidPH = ${sensorData.soidPH} AND soidHumidity = ${sensorData.soidHumidity} AND lightIntensity = ${sensorData.lightIntensity} AND username = ${sensorData.temperature}) LIMIT 1;`;
    db.query(
      query,
      [
