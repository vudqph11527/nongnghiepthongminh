...
module.exports = {
  ...
  createDataSensor: (sensorData, callback) => {
    db.query(
      `INSERT INTO devices (temperature, humidity, soidHumidity,soidPH,lightIntensity) SELECT * FROM (SELECT ?,?,?,?,?) AS tmp WHERE NOT EXISTS (SELECT temperature, humidity,soidHumidity,soidPH,lightIntensity FROM devices WHERE temperature = ${sensorData.temperature} ) LIMIT 1;`,
      [
