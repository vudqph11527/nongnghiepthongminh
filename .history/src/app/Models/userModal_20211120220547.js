...
module.exports = {
  create: (data, callback) => {
    const query = `INSERT INTO users (username, password, phoneNumber) 
    SELECT * FROM (SELECT ?,?,?) 
    AS tmp 
    WHERE NOT EXISTS (SELECT username, phoneNumber FROM users WHERE username = VALUES(username) AND phoneNumber = VALUES(phoneNumber)) LIMIT 1;`;
    db.query(
      query,
      console.log(query)[(data.username, data.password, data.phoneNumber)],
      (err, results, fields) => {
        if (err) {
          callback(err);
        }
