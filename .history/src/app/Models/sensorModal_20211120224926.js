...
module.exports = {
  ...
  createDataSensor: (sensorData, callback) => {
    const query = `INSERT INTO devices (temperature, humidity, soidHumidity,soidPH,lightIntensity) SELECT * FROM (SELECT ?,?,?,?,?) AS tmp WHERE NOT EXISTS (SELECT temperature, humidity,soidHumidity,soidPH,lightIntensity FROM devices WHERE temperature = ${sensorData.temperature} AND humidity = ${sensorData.humidity} AND soidPH = ${sensorData.soidPH} AND soidHumidity = ${sensorData.soidHumidity} AND lightIntensity = ${sensorData.lightIntensity}) LIMIT 1;`;
    db.query(
      query,
      [
